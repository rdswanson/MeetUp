...
public class CourseTime implements Comparable<CourseTime> {
    ...
    /**
     * Return true if the timeOfDay falls between the start and end of a section.
     * @param timeOfDay
     * @return
     */
    public boolean checkIfBusy(String timeOfDay) {

        int sectionStartTime = (this.startHours*60) + this.startMinutes;
        int sectionEndTime = (this.endHours*60) + this.endMinutes;
        int convertedStringTime = (Integer.parseInt(timeOfDay) * 60);

        return ((sectionStartTime <= convertedStringTime) &&
                (convertedStringTime <= sectionEndTime));
    }

    /**
     * Helper function to convert HH:MM formatted time to an integer (minutes since midnight)
     * @param timeOfDay
     * @return
     */
    private static int calculateMinutesIntoDay(String timeOfDay) {
        int colonIndex = timeOfDay.indexOf(":");
        int hours = Integer.parseInt(timeOfDay.substring(0, colonIndex));
        int minutes = Integer.parseInt(timeOfDay.substring(colonIndex + 1,
                timeOfDay.length()));
