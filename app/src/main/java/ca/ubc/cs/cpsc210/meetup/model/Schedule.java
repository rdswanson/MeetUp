...
public class Schedule {
     ...
     * @return The times in HH:MM of the start time of each two-hour break
     */
    public Set<List<String>> getStartTimesOfOneHourBreaks(String dayOfWeek) {

        SortedSet<Section> sections = getSectionsForDayOfWeek(dayOfWeek);
        Set<List<String>> startAndEndTimes = new HashSet<List<String>>();

        if (sections.size() == 1) {

            Section section = sections.first();
            String breakStartTime = section.getCourseTime().getEndTime();
            String breakEndTime = "11:59";
            List<String> startAndEnd = new ArrayList<String>();
            startAndEnd.add(breakStartTime);
            startAndEnd.add(breakEndTime);
            startAndEndTimes.add(startAndEnd);
            ...
            while (it.hasNext()) {
                section = it.next();
                String startTime = lastSection.getCourseTime().getEndTime();
                String endTime = section.getCourseTime().getStartTime();
                List<String> startAndEnd = new ArrayList<String>();

                if (calculateBreakTimeInMinutes(endTime, startTime) >= 60) {
    ...
    public Set<BreakBlock> getStartTimesOfOneHourBreaksRefactored(String dayOfWeek) {

        SortedSet<Section> sections = getSectionsForDayOfWeek(dayOfWeek);
        Set<BreakBlock> startAndEndTimes = new HashSet<BreakBlock>();
        ...
        if (sections.size() == 1) {
            ...
            String breakStartTime = section.getCourseTime().getEndTime();
            String breakEndTime = "11:59";
            BreakBlock breakBlock = new BreakBlock(breakStartTime, breakEndTime);
            startAndEndTimes.add(breakBlock);
            ...
            while (it.hasNext()) {
                ...
                String startTime = lastSection.getCourseTime().getEndTime();
                String endTime = section.getCourseTime().getStartTime();
                BreakBlock breakBlock = new BreakBlock(startTime, endTime);
    ...
    public Building whereAmIRefactored(String dayOfWeek, String timeOfDay) {

        SortedSet<Section> sections = getSectionsForDayOfWeek(dayOfWeek);
        SortedSet<Section> oldSections = new TreeSet<Section>();
        String completeTimeOfDay = timeOfDay + ":00";
        int convertedTimeOfDay = calculateMinutesIntoDay(completeTimeOfDay);
        Section lastSection = null;

        for (Section section : sections) {
            int sectionEndTime = calculateMinutesIntoDay(section.getCourseTime().getEndTime());
            if (convertedTimeOfDay > sectionEndTime) {
                oldSections.add(section);
            }
        }

        if (oldSections.size() > 0) {
            return oldSections.last().getBuilding();
        } else {
            return null;
        }
    }
